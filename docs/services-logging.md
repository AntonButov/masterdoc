...
> Проверено: 2026-07-26 · источники: аудит кода + CI deploy сессии
...
|--------|-------------|------------|
| api-gateway, feature, catalog, document, dashboard, black-box, technologist, technologist-mcp | `/opt/<repo>/` | `docker compose logs -f` |
| backend (`masterdoc-backend`) | systemd | `journalctl -u masterdoc-backend -f` |

Отдельных файлов `/var/log/...` в деплое нет.

## Что залогировано (2026-07-26)
...
| dashboard-service | `fc900e4` main | `catalog_lookup_failed`, `scheduler_tick`/`scheduler_failed` |
| black-box-service | `9ae54c0` main | `auth_denied`, `audit_appended` |
| technologist-service | `0c8ced3` main | `job_*`, `document_text_fetch`, `mcp_call`, `package_confirmed` |
| technologist-mcp | `a93b942` main | `tool_call`/`tool_denied`/`tool_error`/`upstream_call` |
