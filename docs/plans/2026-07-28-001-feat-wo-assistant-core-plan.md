...
- **Objective:** Make the former «Наставник» / `copilot` a core engineer capability: a multi-turn assistant dialog on the engineer's assigned work order, grounded in that asset's documentation via document-mcp — not a grantable feature or nav destination. Hard-enforce that dispatchers do not perform work orders.
- **Product authority:** This Product Contract (brainstorm 2026-07-28) + Planning Contract KTDs below. Adjacent: features-only ACL (`equipment` for engineers); engineer-scope binding plan; document-mcp agent pattern; `masterdocapp` Copilot out of identity.
- **Open blockers:** None.
...
- **Stop when:** AE1–AE4 hold; `copilot` gone from catalog/nav; `/ai/mentor` works for assignee+`equipment`; board-only user cannot be WO assignee.
...
Engineers execute work orders under `equipment` access.
The assistant is inseparable from that work: a button on an assigned WO opens a multi-turn dialog whose answers use the WO fields plus the asset's documents through existing document-mcp tools.
...
- **KD1. Assistant lives on the assigned work order** `(session-settled: user-directed — chosen over nav tab / global panel / asset-card-only: engineer work is the WO)`. Button → dialog; no persistent assistant panel. No assignee WO → no assistant UI.
...
- **KD3. Stops being a feature** `(session-settled: user-directed — chosen over keeping `copilot` as hidden access key or leaving nav: assistant is not entitlement)`. Remove `copilot` from catalog, invites, IdP role keys, and client nav. Engineer product access is `equipment` (WO work); assistant is always part of that surface.
- **KD4. Working dialog in v1** `(session-settled: user-directed — chosen over stub/placeholder or catalog-only change: success = answer grounded in asset docs)`.
...
- **KD7. Dispatchers do not perform work orders — hard enforce** `(session-settled: user-directed — chosen over process-only: «жёстко диспетчер не может выполнять заявку»)`. System rejects making a board-only (non-`equipment`) user the WO assignee; assignee/performer is engineer/`equipment` territory.
- **KD8. Multi-turn while dialog open** `(session-settled: user-directed — chosen over one-shot: «несколько вопросов подряд»)`. In-session history for the open dialog; no durable cross-session product memory in v1.
...
- **A1. Engineer** — has `equipment`; is (or becomes) WO `assigneeId`; primary user of the assistant button and dialog.
...
- **A3. Assistant agent** — answers in the WO dialog using WO context + document-mcp over the WO's asset.
...
- F1. Open assistant from assigned WO
  ...
  - **Actors:** A1, A3
  - **Steps:** UI shows start-assistant control; engineer opens dialog; asks one or more questions in the same open dialog; agent uses WO fields + document-mcp for the WO's asset; returns grounded answers.
...
- F2. No assignment → no assistant
  ...
  - **Actors:** A1
  - **Steps:** No assistant button / dialog entry for that context.
...
- F3. Entitlement cleanup
  ...
  - **Steps:** `copilot` / «Наставник» is not offered; engineer is granted `equipment`.
  - **Outcome:** Assistant cannot be toggled off as a feature.
  - **Covered by:** R7, R8

- F4. Dispatcher cannot perform WO
  - **Trigger:** Attempt to set WO `assigneeId` to a user without `equipment` (board-only dispatcher).
  - **Actors:** A2
...
```mermaid
flowchart LR
  eng[Engineer with equipment]
  wo[WO where assigneeId = eng]
  btn[Start assistant dialog]
...
- R1. On a work order where the current user is `assigneeId`, the client shows a control to start an assistant dialog (not a permanent side panel, not a top-level «Наставник» destination).
- R2. The dialog is a working multi-turn assistant in v1 while open: the engineer can ask several questions and receive answers grounded in that WO's fields and the linked asset's documentation.
...
- R9. Engineer access to perform work orders is expressed with `equipment` (and existing board-read rules updated so they do not depend on `copilot`).
- R10. Org or user cannot disable the assistant as a product feature; closing the dialog is fine, removing the capability is not.

**Roles**

- R11. Hard rule: dispatchers do not perform work orders — a user without `equipment` must not become WO `assigneeId`; assignment targets engineers.

### Acceptance Examples

- AE1. Covers R1, R2, R5
  - **Given:** Engineer E is `assigneeId` on WO W for asset A created from docs (docs reachable via document-mcp).
  - **When:** E opens W, starts the assistant dialog, and asks two follow-up questions about steps in A's docs.
  - **Then:** Dialog stays multi-turn while open; replies are usable and grounded in A's documentation.
...
- AE3. Covers R7, R8, R10
  ...
  - **Then:** `copilot` / «Наставник» is not selectable; `equipment` suffices for WO work including assistant on assigned WOs.

- AE4. Covers R11
  - **Given:** Dispatcher D has `board` only (no `equipment`).
...
- An engineer on their assigned WO can hold a multi-turn dialog and get answers that use the asset's documentation.
...
- WO-bound assistant button + multi-turn dialog in client-app engineer flow.
- Remove `copilot` feature and «Наставник» nav; engineer ACL via `equipment`.
...
- Engineer WO list/read access must not require `copilot` after removal (supersede `equipment|copilot` mentions in `masterdoc/docs/plans/2026-07-27-001-feat-engineer-scope-binding-plan.md`).
- Dual-grant `board`+`equipment` users may be assignees (they have `equipment`).
...
- Q2 → KD7 / KTD2 (hard enforce assignee must have `equipment`).
- Q3 → KTD3 (strip on read + remove catalog/terraform/fixtures).
- Q4 → KTD4 (ignore `masterdocapp` Copilot for this delivery).

**Deferred to implementation**

- Exact Compose dialog chrome (modal vs full-screen sheet) — pick consistent with existing client patterns when added.
...
- KTD2. **Assignee must have `equipment`** — On PATCH assignee (non-null), dashboard rejects if the **target** user (`assigneeId`) lacks `equipment`, via feature-service (or equivalent) lookup by that userId — never the caller's feature header alone. Clearing assignee remains allowed. UI assignee picker excludes board-only users when possible.
- KTD3. **Remove `copilot` everywhere it is a product feature** — Catalog, gateway `ProductFeatures`, OpenAPI enums, Zitadel terraform role key + invite workflows, client `FeatureId.Copilot` / nav / labels / fixtures. Unknown leftover IdP grants drop via catalog filter. WO GET read features become `board` + `equipment` only. `canAccessWorkOrderBoard` drops `Copilot`.
- KTD4. **`masterdocapp` Copilot ignored** — No deep-link; no change required to ship this plan.
- KTD5. **Gateway proxy** — `proxyPrefix("/ai/mentor", technologistServiceBaseUrl, features = listOf("equipment"))`. Server still asserts caller is WO assignee.
- KTD6. **Client dialog** — On `WorkOrderDetailScreen`, show start control iff `assigneeId == session.user.id`. Multi-turn in-memory for the open dialog; each send posts `{ workOrderId, message, history? }` (or server session id if cheaper — implementer choice, v1 not durable across app restarts). Non-streaming JSON reply first (match other `/ai/*` clients).
- KTD7. **Unassign / lose assignee mid-dialog** — Next send fails; client closes/hides dialog and removes control.
...
- Dual `board`+`equipment` may be assignee.
...
- Engineer-scope plan text that cites `equipment|copilot` for WO reads should be treated as superseded by `equipment` alone.
...
5. U6 hard assignee=`equipment` enforce (can parallel after U1 feature set clarity)
...
- **Goal:** `copilot` is no longer a grantable/readable product feature; WO reads work on `board`|`equipment`.
...
- **Approach:** Delete catalog entry and mirrors; WO `readFeatures = board, equipment`; remove copilot from terraform `locals.roles` and INVITE_ROLE_KEYS; update tests.
...
- **Test scenarios:**
  ...
  - GET `/work-orders*` allowed with `equipment` without `copilot`.
...
- **Goal:** No Copilot destination; board access via board|equipment; engineer fixtures use `equipment`.
...
- **Approach:** Remove `FeatureId.Copilot` / nav spec; `canAccessWorkOrderBoard` = Board || Equipment; replace `engineerCopilot()` fixture with `engineerEquipment()`; drop «Наставник» label.
- **Test scenarios:**
  - Covers AE2. Engineer equipment fixture builds nav without Copilot item; board reachable via equipment.
...
- **Goal:** Client reaches mentor only with `equipment`.
- **Requirements:** R9, R10; KTD5
- **Dependencies:** U3
- **Files:**
  - `api-gateway-service/src/main/kotlin/pro/masterdoc/gateway/EquipmentRoutes.kt`
  - gateway proxy tests for AI routes if present / extend
- **Approach:** Mirror `proxyPrefix` for technologist/validator/card with `features = listOf("equipment")`.
- **Test scenarios:**
  - Request without `equipment` feature denied.
  - Request with `equipment` proxied to technologist base URL path `/ai/mentor`.
- **Verification:** gateway tests green.

### U5. Client WO assistant button + multi-turn dialog

- **Goal:** Assignee sees start control and can chat multi-turn grounded via `/ai/mentor`.
...
- **Files:**
  ...
  - equipment/AI repository module (extend pattern from `EquipmentModels.kt` AI posts)
  - new dialog composable + tests under `composeApp` / `shared` as appropriate
- **Approach:** Show button iff assignee; dialog holds local message list; send appends user+assistant turns; on auth/assignee error close dialog. Do not add global nav entry.
...
- **Test scenarios:**
  ...
  - Mentor 403 mid-dialog → dialog closes / control hidden when assignee cleared.
...
- **Files:**
  ...
  - client assignee picker if present (Board dispatcher UI) — exclude non-equipment candidates when list available
  - gateway only if feature set must be forwarded for the check
- **Approach:** When `assigneeId` set non-null, verify target has `equipment`; reject otherwise with clear error. Keep existing scope `covers` check. Clearing assignee OK.
- **Execution note:** Test-first on PATCH assignee rejection.
- **Test scenarios:**
  - Covers AE4. Assign board-only user → rejected.
  - Assign equipment user in scope → accepted.
...
- **Cross-plan:** After U1, engineer WO reads must succeed with `equipment` alone (align with engineer-scope binding intent).
...
- `POST /ai/mentor` reachable via gateway for `equipment` assignees; document-mcp read tools used.
